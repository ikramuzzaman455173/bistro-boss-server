...
app.get("/", (req, res) => {
  res.send(
    `<h1 align="center" style="color:#333;font-size:20px;margin:10px 0;">Bistro Boss Server Is Runnings</h1>`
  );
});
...
async function run() {
  try {
    ...
    const bistroBossCollection = client
      .db("bistroBossDb")
      .collection("bistroMenu");
    const reviewsBossCollection = client
      .db("bistroBossDb")
      .collection("reviews");
    const cartsBossCollection = client.db("bistroBossDb").collection("carts");
    const paymentsBossCollection = client
      .db("bistroBossDb")
      .collection("payments");
    ...
    app.post("/jwt", (req, res) => {
      ...
      const token = jwt.sign(user, process.env.access_token_secreat_key, {
        expiresIn: "1h",
      });
      res.send({ token });
    });
    ...
    app.get("/admin-stats", varifyJwt, varifyAdminJwt, async (req, res) => {
      ...
      const revenue = payments.reduce(
        (sum, money) => sum + money.payment.price,
        0
      );
    ...
    app.post("/payments", varifyJwt, async (req, res) => {
      ...
      const query = {
        _id: { $in: payment.payment.cartItems.map((id) => new ObjectId(id)) },
      };
